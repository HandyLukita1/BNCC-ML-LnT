...
with st.form('form_obesitas'):
  ...
  with col1:
    ...
    height = st.number_input('Tinggi Badan', min_value=1, max_value =2.5, value=1.70)
...
if submit_button:
  input_data = pd.DataFrame({
      ...
      'FAVC (Frequent High Calorie Food)':[favc],
      'FCVC (Frequent consumption of vegetables)':[0],
      'NCP (Number of main meals)':[0],
      'CAEC (Snacking)': ['Sometimes'],
      'SMOKE':['no'],
      'CH2O (Consumption of water in liters)':[2],
      'SCC (Consumption of food between meals)':['no'],
      'FAF (Frequent of Physical Activity)':[faf],
      'TUE (Frequent Use of Gadget)':[1],
